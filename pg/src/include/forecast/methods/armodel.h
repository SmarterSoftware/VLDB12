...
typedef struct ARModel
{
	...
	int	maint;		//flag to indicate if the data array should be free-ed
}ARModel;
...
extern void identifyArimaModel(ARModel *model);
...
extern void updateArUhat(double* uhat, double forecast,double realValue, int uhatLength);
extern void expandYArray(ArimaModel *forecastModel,int to);

extern double calculateArTrustedInterval(Model *model, int k, double percentage);
/*
 * utility functions
 */
#endif /* COMMONHW_H_ */
