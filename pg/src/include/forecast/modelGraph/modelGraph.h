...
extern List*			modelsToReestimate;
...
void
setFullFillingSourceText(char *sourceText, int length);

char *
getFullFillingSourceText(void);
...
void
MaintainModelGraphAggs(Oid relId);
...
void
freeDisAggModel(DisAggModel *dam);
...
int
FindModelGraphModels(ModelInfo* modelinfo,List **resultList,List **possiblePaths, List **createModels,List *rtable,Node **whereExpr, int explicitCreateModel);
...
void triggerMaintenance(int32 number, int32 strategy);
