...
typedef struct ModelGraphIndex{
	...
	//saves the period for the modeled timeseries, a possible fallback for nonseasonal timeseries could be the errorArrayLength
	//(possibly we need this as a bound for ParameterReestimation and GreedyRestart)
	int					reBound;

	//a workingfield for the calculation of the standardized configurationError
	double				*confErr;

	//how many INSERTs may affect the modelGraph until Greedy is restarted
	int					restartInsCnt;

	//how many INSERTs affected the modelGraph since Greedy
	int 				insertCnt;

	//the reference value for triggering  restart of Gredy
	double				referenceGreedyEval;

	//all leafs of the ModelGraph are accessible here
	ModelGraphIndexNode	**leafArray;
}ModelGraphIndex;


#define foreachModelInMG(i, modelinfo)	\
	for (i=0, (modelinfo) = modelGraphIdx->modellist[i]; i<modelGraphIdx->models; ++i, (modelinfo) = modelGraphIdx->modellist[i])
