...
CreateForecastModelState *ExecInitCreateForecastModel(CreateForecastModel *node, EState *estate, int eflags)
{
	...
	if ((createState->modelInfo->storeModel == 0) || (createState->modelInfo->storeModel == 10)||(createState->modelInfo->storeModel == 2))
		...
		initForecastModel(createState->modelInfo, getModelGraphFillingContext());
	else
	...
	if(node->algorithmStmt != NULL && ((AlgorithmClause *)node->algorithmStmt)->trainingdata){
		outerPlanState(createState) = ExecInitNode(trainingPlan, estate, eflags);
	}
	else
	{
...
TupleTableSlot *ExecCreateForecastModel(CreateForecastModelState *createState)
{
		...
		while((outSlot = ExecProcNode(trainingPlan)) && !TupIsNull(outSlot)) //we have a tuple so process it
		{
			//count tuples
			createState->count++;
			...
			// check granularity
			if(!time1)//save the 1st timestamp
			{
				time1 = GetDatumAsInt(exprType((Node*) createState->modelInfo->time->expr), slot_getattr(outSlot, createState->modelInfo->time->resno, &isnull));
			}
			else if(!time2)//save the 2nd timestamp
			{
				time2 = GetDatumAsInt(exprType((Node*) createState->modelInfo->time->expr), slot_getattr(outSlot, createState->modelInfo->time->resno, &isnull));
				createState->modelInfo->granularity = extractGranularity(exprType((Node*) createState->modelInfo->time->expr), time1, time2);
			}else{
				time1=time2;
				time2 = GetDatumAsInt(exprType((Node*) createState->modelInfo->time->expr), slot_getattr(outSlot, createState->modelInfo->time->resno, &isnull));
				switch(exprType((Node*) createState->modelInfo->time->expr))
				{
					case TIMESTAMPOID:
						diffInterball=extractGranularity(TIMESTAMPOID, (time1), (time2));
						if(diffInterball!=createState->modelInfo->granularity)
						{
						//	elog(ERROR,"Given timeseries is not equidistant or has more than one entry per timestamp");
						}
						break;
					case DATEOID: 
						diffInterball=extractGranularity(DATEOID, (time1),  (time2));
						if(diffInterball!=createState->modelInfo->granularity)
						{
							//elog(ERROR,"Given timeseries is not equidistant or has more than one entry per timestamp");
						}
		...
		if(!sourceCpy) // implicit model creation!
		{
			...
			if(!sourceCpy){
				...
				if(!sourceCpy2)
				{
					sourceCpy2=sourceCpy4;
					sourceCpy2=strrstr(sourceCpy2,"asof");
				}
				ptr5=sourceCpy2;
