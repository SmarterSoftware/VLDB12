...
double	*measureConfErr = NULL;

ModelGraphIndexNode *modelGraphRoot = NULL;
ModelGraphIndex *modelGraphIdx = NULL;

char *graphCreationSourceText = NULL;
char *fullGraphCreationSourceText = NULL;
char *fullFillingSourceText = NULL;

//memoryContext of the tree himself
MemoryContext 	metalGraphContext = NULL,

// memory context for the filling of the modelGraph
				modelGraphFillingContext = NULL,
                oldContext;
...
void setFullFillingSourceText(char *sourceText, int length){
	fullFillingSourceText = palloc0((sizeof(char)+1)*length);
	fullFillingSourceText = strcpy(fullFillingSourceText, sourceText);
}

char *getFullFillingSourceText(void){
	return fullFillingSourceText;
}
 ...
 */
ModelGraphIndexNode *
getModelGraphRoot(bool forFilling) {

	if(!modelGraphRoot) {
		metalGraphContext = AllocSetContextCreate(TopMemoryContext,
		                    "ModelGraphContext",
		                    ALLOCSET_DEFAULT_MINSIZE,
		                    ALLOCSET_DEFAULT_INITSIZE,
		                    ALLOCSET_DEFAULT_MAXSIZE);
		oldContext = MemoryContextSwitchTo(metalGraphContext);
		...
		//only switch MemoryContext if the current MemoryContext is NOT the modelGraphFillingContext
		int i=strcmp(CurrentMemoryContext->name, "ModelGraphFillingContext");
		if(i!=0)
			oldContext = MemoryContextSwitchTo(metalGraphContext);
	}
	...
	if(!modelGraphFillingContext)
	{
		modelGraphFillingContext= AllocSetContextCreate(metalGraphContext,
		"ModelGraphFillingContext",
		ALLOCSET_DEFAULT_MINSIZE,
		ALLOCSET_DEFAULT_INITSIZE,
		ALLOCSET_DEFAULT_MAXSIZE);
	}	
	if(forFilling){
			MemoryContextSwitchTo(modelGraphFillingContext);
	}
...
MemoryContext
getModelGraphFillingContext(void) {
	return modelGraphFillingContext;
}
...
getSourceText(void) {
	...
	Assert(modelGraphFillingContext);
	oldContext = MemoryContextSwitchTo(modelGraphFillingContext);
...
//TODO: this should be something other, because if we don't hit it could mean we entered a wrong value as predicate
			...
			case 1042: {
				mgin2 = arlist_get(mgin2->children,find(mgin2->children, (void *)DatumGetPointer(searchHit), Mgin_Char_SorterAsc));
				if(!mgin2)
			...
			case 1043: {
				mgin2 = arlist_get(mgin2->children,find(mgin2->children, (void *)DatumGetCString(searchHit), Mgin_Char_SorterAsc));
				if(!mgin2)
...
AddModelToModelGraph(ModelInfo *modelInfo, List *rTable) {
	...
	ModelGraphIndexNode *mgin = getModelGraphRoot(true),
	                     *mgin2;
...
CreateModelGraph(CreateModelGraphStmt *stmt) {
	...
	int 				attributeCount = 0, //counts how many levels the graph will have
	                    i;
	ModelGraphIndexNode *mgin = getModelGraphRoot(false),
	                     *mgin2;
...
modelGraphCompatible(Node* whereExpr, Query *qry) {
	ModelGraphIndexNode *root = getModelGraphRoot(true);
	List *tgel = root->targetEntryList;
	List *tgelQuery=qry->targetList;
	ListCell *lc,*lc2;
	RangeTblEntry *rte;
	int found=0;
	Var *opvar;
	...
	foreach(lc,tgel) {
		found=0;
		TargetEntry *teM=lfirst(lc);
		foreach(lc2,tgelQuery){
			TargetEntry *teQ=lfirst(lc2);
			if(teM->resorigtbl != teQ->resorigtbl)
				continue;
			//found a matching TargetEntry
			found=1;
			break;
...
//XXX:should work this way, previously there was a distinction bewtween Aggref and Var, but I think this is NOT important, the mistake must be somewhere else
	...
	disAggModel->upperBound = disAggModel->model->upperBound;
	disAggModel->lowerBound = disAggModel->model->lowerBound;
	disAggModel->sizeOfErrorArray = 0;
	disAggModel->errorArray = palloc0(disAggModel->upperBound*sizeof(double));
...
static void
_setModelGraphArray(ModelGraphIndexNode *mgin)
{
	ArListCell *alc;
	if(!mgin->children) {
...
SetModelGraphArray(void)
{
	...
	_setModelGraphArray(getModelGraphRoot(false));
	releaseModelGraphRoot();
}
...
FillModelGraph(FillModelGraphStmt *stmt) {
	...
	oldCtx = MemoryContextSwitchTo(metalGraphContext);
	setFullFillingSourceText((char *)stmt->sourcetext, strlen(stmt->sourcetext));
	...
	case 0:	//TOPDOWN
		...
		break;
	case 1: //BOTTONUP
		FillModelGraphBottomUp(stmt, false);
		break;
	case 2: //GREEDY
		FillModelGraphGreedy(stmt);
		break;
	case 3: //COMPLETE
		FillModelGraphBottomUp(stmt, true);
		break;
	}

	clock_gettime(CLOCK_MONOTONIC, &end);

	uint64_t timeElapsed = (((&end)->tv_sec * 1000000) + (&end)->tv_nsec/1000) - (((&start)->tv_sec * 1000000) + (&start)->tv_nsec/1000);
	elog(INFO, "Time elapsed: %lu ms\n", timeElapsed/1000);
}
...
FillModelGraphTopDown(FillModelGraphStmt *stmt) {
	...
	forecast->maintenancetype=stmt->maintenanceMethod;
	forecast->maintenanceParameters=stmt->maintenanceParameters;
...
void
FillModelGraphBottomUp(FillModelGraphStmt *stmt, bool complete) {
	...
	nodesConsidered = GetAllChildren(NULL, complete);
	...
	foreach(lc, nodesConsidered){
		...
		//((SelectStmt *)((ForecastExpr *)selStmt->forecastExpr)->algorithm->trainingdata)->whereClause = (Node *)aexp;
...
void freeDisAggModel(DisAggModel *dam){

	pfree(dam->errorArray);
...
void EmptyModelGraph(void){
	...
	foreach(lc, allChildren)
	{
		...
		mgin->lastInsertedTuple = NULL;
	}


	if(CurrentMemoryContext == modelGraphFillingContext)
		MemoryContextSwitchTo(metalGraphContext);

	if(modelGraphFillingContext)
		(*modelGraphFillingContext->methods->delete)(modelGraphFillingContext);

	modelGraphFillingContext = NULL;

	modelGraphIdx->confErr = NULL;
...
//	if(modelGraphIdx->modellist)
//		pfree(modelGraphIdx->modellist);
	modelGraphIdx->modellistMaxSize = 0;
	modelGraphIdx->models = 0;
}
...
void MaintainModelGraph(HeapTuple newTuple, Relation sourceRelation){
	...
	DisAggModel			*dam = NULL;
	Var					*v;
	double				f,
						t,
						error = 0.0,
						bestError = 0.0,
						measureBestError = 0.0;
	...
	if(modelGraphIdx->confErr == NULL)
		modelGraphIdx->confErr = palloc0(sizeof(double)*(modelGraphIdx->maxid+1));
	...
	foreach(lc, mgin->models)
	{
		...
		measureBestError=(*(model->maintenance->insert))(model,t);

		//of the model is not marked for reestimation, update its state
		if(!list_member_ptr(modelsToReestimate, model))
	...
	if((!model || (dam&&dam->errorSMAPE<model->errorSMAPE)) && mgin->disAggModels)
	{
		bestError = dam->errorSMAPE;
		measureBestError = error;
	}
	...
	if(!mgin->lastInsertedTuple)
		mgin->lastInsertedTuple = (Datum *) palloc0(tDesc->natts * sizeof(Datum));

	for(i=0; i<tDesc->natts; ++i)
	{
		//as I understand it I free a small (reusable) chunk of the right size...
		if(mgin->lastInsertedTuple[i])
		{
			datumFree(mgin->lastInsertedTuple[i], tDesc->attrs[i]->attbyval, tDesc->attrs[i]->attlen);
		}
		//...so the datumCopy should be able to reuse the chunk here, because there is no change of MemoryContext between these two loc
		mgin->lastInsertedTuple[i] = datumCopy(values[i],tDesc->attrs[i]->attbyval, tDesc->attrs[i]->attlen);
	}

	if(!measureConfErr)
		measureConfErr = palloc0((modelGraphIdx->maxid+1)*sizeof(double));

	measureConfErr[mgin->id] = measureBestError/(modelGraphIdx->maxid+1);
	modelGraphIdx->confErr[mgin->id] = bestError/(modelGraphIdx->maxid+1);
	StringInfo data = makeStringInfo();
	appendStringInfo(data, "%f\t", measureBestError);
...
void MaintainModelGraphAggs(Oid relId){
	...
	//if no Modelgraph was created OR there are no models -> there is nothing to update
	if(!mgin->children || modelGraphIdx->models==0)
		return;

	if(relId != ((ModelGraphIndexNode *)arlinitial(mgin->children))->target->resorigtbl)//the inserted Tuples don't affect the relation the ModelGraph was created for
		return;
	...
	foreach(lc1,nodeList)
	{
		...
		//TODO: later we have to respect different measurcolumns of different models here
		//-> _getModel maybe can help to achieve this, but it can be hard to manage this for normal Models AND DisAggModels
		t=0;
		if (mgin->models)
		{
			double *temp;
			...
			{
				int arrayLength = 0;
				double disAggKeyNumerator=0;
				ModelGraphIndexNode *disAggTargetNode=(ModelGraphIndexNode *)lfirst(lc3);

				if(disAggTargetNode->aggIndicator<1)
					t += GetDatumAsDouble(v->vartype, disAggTargetNode->lastInsertedTuple[v->varoattno-1]);




				//MEASURE-CHANGE
				//monitoring all possible disAgg-Errors
				...
				{
					double temp;
					f = getNextValue(model, 1)*(disAggKeyNumerator-(DatumGetFloat8(disAggTargetNode->lastInsertedTuple[v->varoattno-1])))/(model->disAggKeyDenominator);

					temp = SMAPE(f, DatumGetFloat8(disAggTargetNode->lastInsertedTuple[v->varoattno-1]));
					if(model->sizeOfErrorArray==model->upperBound)
					{
						memmove(&(model->otherErrors[disAggTargetNode->id][0]),&(model->otherErrors[disAggTargetNode->id][1]),(model->sizeOfErrorArray-1)*sizeof(double));
						model->otherErrors[disAggTargetNode->id][model->sizeOfErrorArray-1]=temp;
						arrayLength = model->sizeOfErrorArray;
					}
					else
					{
						model->otherErrors[disAggTargetNode->id][model->sizeOfErrorArray]=temp;
						arrayLength = model->sizeOfErrorArray+1;
					}
				}

				temp = model->otherErrors[disAggTargetNode->id];
				errorSMAPE = meanOfDoubleArray(temp, arrayLength);

				if(disAggTargetNode->disAggModels)
				{
					dam = (DisAggModel *)linitial(disAggTargetNode->disAggModels);
					//the actually considered model has an better disaggregationError than the model used at this time
					if(errorSMAPE < ((DisAggModel *)linitial(disAggTargetNode->disAggModels))->errorSMAPE &&
							((DisAggModel *)linitial(disAggTargetNode->disAggModels))->model != model &&
							allowSwapping)
					{



						//the disaggregation is really used because it's better than the (maybe not existent) model
						if(disAggTargetNode->models == NULL
								|| dam->errorSMAPE<((ModelInfo *)linitial(disAggTargetNode->models))->errorSMAPE)
						{
							//remember the currently used model to be the second best...
							model->errorsForDropingDecision[disAggTargetNode->id] = dam->model;
							//...and let it self forget its second best
							dam->model->errorsForDropingDecision[disAggTargetNode->id] = NULL;
						}



						//from now use model as the new disaggregation source
						dam->model = model;
//						if(dam->sizeOfErrorArray==model->sizeOfErrorArray)
//						{
//							dam->errorArray[dam->sizeOfErrorArray-1] = model->otherErrors[disAggTargetNode->id][model->sizeOfErrorArray-1];
//						}
//						else
//						{
//							dam->errorArray[dam->sizeOfErrorArray-1] = model->otherErrors[disAggTargetNode->id][model->sizeOfErrorArray];
//						}
						memcpy(dam->errorArray, model->otherErrors[disAggTargetNode->id], maintTplCnt*sizeof(double));
						dam->errorSMAPE = meanOfDoubleArray(dam->errorArray, dam->sizeOfErrorArray);

					//model is not better than the current disaggregation but it's the second best or the first considered since Greedy
					}
					else
					{

						if(dam->model->errorsForDropingDecision[disAggTargetNode->id] == NULL
								|| errorSMAPE < meanOfDoubleArray(dam->model->errorsForDropingDecision[disAggTargetNode->id]->otherErrors[disAggTargetNode->id], arrayLength))
						{
							...
							if((disAggTargetNode->models == NULL
									...
									&& dam->model != model)
							{
								dam->model->errorsForDropingDecision[disAggTargetNode->id] = model;

								//this shouldn't be necessary (do it for safety)
								model->errorsForDropingDecision[disAggTargetNode->id] = NULL;
							}
							//there never was a secondbest model found for this disaggregation...
						}
						else if(dam->model->errorsForDropingDecision[disAggTargetNode->id] == NULL)
						{
							//...so if there is a model that is not better than the disaggregation set this the secondbest
							if(disAggTargetNode->models && dam->errorSMAPE < ((ModelInfo *)linitial(disAggTargetNode->models))->errorSMAPE){
								memcpy(((ModelInfo *)linitial(disAggTargetNode->models))->otherErrors[disAggTargetNode->id], ((ModelInfo *)linitial(disAggTargetNode->models))->errorArray, ((ModelInfo *)linitial(disAggTargetNode->models))->upperBound);
								dam->model->errorsForDropingDecision[disAggTargetNode->id] = ((ModelInfo *)linitial(disAggTargetNode->models));
							}
							else
								elog(INFO,"We should never reach this point except the disaggregation is from %i and it is from %i.", modelGraphIdx->maxid, disAggTargetNode->id);
						}
					}
				}

				//if disAggTargetNode contains models...
				if(disAggTargetNode->models){
					ModelInfo *localModel = (ModelInfo *)linitial(disAggTargetNode->models);

					//...and the actually considered model has an better disaggregation
					if(dam && dam->errorSMAPE > localModel->errorSMAPE)
					{
						localModel->errorsForDropingDecision[disAggTargetNode->id] = dam->model;
					}
				}
			}
		}
		...
		if(t>0)
		{
			//this is kind of fake, the basetimeseries save the full new tuple, aggregated timeseries only save the newest measure-value
			if(!(mgin->lastInsertedTuple))
				mgin->lastInsertedTuple = palloc0(sizeof(double)*(v->varoattno));

			if(mgin->lastInsertedTuple[v->varoattno-1])
			{
				get_typlenbyval(v->vartype, &typLen, &typByVal);
				datumFree(mgin->lastInsertedTuple[v->varoattno-1], typByVal, typLen);
			}
...
//TODO:support several Models in every Node
		if (mgin->models)
		{
			...
			//MEASURE-CHANGE
			//calculate the LCP-Error
			if(mgin->id==modelGraphIdx->maxid)
			{
				double tempCount = 0;
				for(nodeID=0;nodeID<mgin->id;++nodeID)
				{
					tempCount = 0;

					for(arraySize=0; arraySize<model->sizeOfErrorArray; ++arraySize)
					{
						tempCount += model->otherErrors[nodeID][arraySize];
					}

					lcpError += (tempCount/arraySize);
					measureLcpError += model->otherErrors[nodeID][arraySize-1];
				}
				...
				for(arraySize=0; arraySize<model->sizeOfErrorArray; ++arraySize)
				{
					tempCount += model->errorArray[arraySize];
				}

				lcpError += (tempCount/arraySize);

				measureLcpError += model->errorArray[arraySize-1];

				lcpError /= (nodeID+1);
				measureLcpError /= (nodeID+1);
		...
		if (mgin->disAggModels)
		{
			...
			//update the own disaggKeyNumerator
			dam->disAggKeyNumerator += t;

			if(!model || dam->errorSMAPE < model->errorSMAPE)
			{
				bestError = dam->errorSMAPE;
				measureBestError = error;
			}
		...
		measureConfErr[mgin->id] = measureBestError/(modelGraphIdx->maxid+1);
		StringInfo data = makeStringInfo();
		appendStringInfo(data, "%lf\t", measureBestError);
		printDebug("/home/hartmann/Desktop/BelegSVN/ClaudioHartmann/data/Potentialanalyse/potentialAnalyse.txt",data->data);
		pfree(data);
	}

	printDebug("/home/hartmann/Desktop/BelegSVN/ClaudioHartmann/data/Potentialanalyse/potentialAnalyse.txt", "\n");
...
//	confErr = palloc0(sizeof(double)*list_length(evalConf));
//	i = 0;

	workingArray = palloc0(sizeof(double)*(modelGraphIdx->maxid+1));

	//calculate the current result of the GreedyEvaluation-formula
	currentGreedyEval = EvaluateConfiguration(modelGraphIdx->confErr, lcpError, modelGraphIdx->models-1, modelGraphIdx->maxid);

	measureCurrentGreedyEval = EvaluateConfiguration(measureConfErr, measureLcpError, modelGraphIdx->models-1, modelGraphIdx->maxid);

	for(i2=0; i2<modelGraphIdx->maxid+1; ++i2)
		tempGreedyEval += measureConfErr[i2];

	elog(INFO, "$$$$$$$$$$$$$$$$$$$$$$$$$$$$$ %lf %lf", tempGreedyEval, measureLcpError);

	StringInfo data = makeStringInfo();
	appendStringInfo(data, "%lf\n", measureLcpError);
	printDebug("/home/hartmann/Desktop/BelegSVN/ClaudioHartmann/data/Potentialanalyse/lcpError.txt",data->data);
	pfree(data);
	elog(INFO, "lcpError: %lf", lcpError);

	elog(INFO, "currentGreedyEval: %lf", currentGreedyEval);

	/*
	 * the errorArrays of the Models have been filled now till the upperBound for the first time
	 * set the currentGreedyEval as the referenceValue to trigger a restart of the GreedyAlgo
	 */
	if(modelGraphIdx->insertCnt+1 == modelGraphIdx->reBound)
	{
		if(modelGraphIdx->referenceGreedyEval)
		{
			elog(INFO, "Greedy alt: %lf, aktuell: %lf", modelGraphIdx->referenceGreedyEval, currentGreedyEval);
			if(currentGreedyEval < modelGraphIdx->referenceGreedyEval)
			{
				elog(INFO, "Greedy häufiger neustarten, war %i", modelGraphIdx->restartInsCnt);
				if(modelGraphIdx->restartInsCnt > 2*modelGraphIdx->reBound)
					modelGraphIdx->restartInsCnt /= 2;
			}
			else
			{
				elog(INFO, "Greedy seltener neustarten, war %i", modelGraphIdx->restartInsCnt);
				modelGraphIdx->restartInsCnt *= 2;
			}
		}

		modelGraphIdx->referenceGreedyEval = currentGreedyEval;
	}

	if(getFullFillingSourceText()!=NULL &&
			(((currentGreedyEval > modelGraphIdx->referenceGreedyEval*(1+(0.05/alpha))) && modelGraphIdx->insertCnt >= modelGraphIdx->modellist[0]->upperBound) ||
					modelGraphIdx->insertCnt+1 >= modelGraphIdx->restartInsCnt) &&
			allowRefill)
	{

//		if((modelGraphIdx->referenceGreedyEval && (currentGreedyEval > modelGraphIdx->referenceGreedyEval*(1+(0.05/alpha)))))
//			elog(INFO, "Restart because of bad Greedy-Value");
//		else
//			elog(INFO, "Restart because of time");

		modelGraphIdx->referenceGreedyEval = currentGreedyEval;

		EmptyModelGraph();

//		elog(INFO, "##################################################Now we restart Greedy");
		runQuery(getFullFillingSourceText(), "FILL MODELGRAPH");

		elog(INFO, "RESTARTED DONE");

		modelGraphIdx->insertCnt = 0;
	...
	memcpy(workingArray, modelGraphIdx->confErr, sizeof(double)*(modelGraphIdx->maxid+1));

	//check for every Model in the actual configuration if it is enough beneficial to keep it
	if (allowDropping) {
		foreachModelInMG(i, model)
		{
			//skip the lcpModel, this is never dropped
			if(((ModelGraphIndexNode *)model->mix)->id == modelGraphIdx->maxid)
			{
				MemSet(model->errorsForDropingDecision, 0, (modelGraphIdx->maxid+1)*sizeof(ModelInfo *));
				continue;
			}
			for(i2=0; i2<(modelGraphIdx->maxid+1); ++i2)
			{
				if(model->errorsForDropingDecision[i2])
				{
		//				int i3;
		//				double err = 0.0;
		//				for(i3=0; i3<model->sizeOfErrorArray-1; ++i3){
		//					err+=model->errorArray[i3];
		//				}
		//				err+=((ModelInfo *)model->errorsForDropingDecision[i2])->otherErrors[i2][model->sizeOfErrorArray-1];
		//				err/=model->sizeOfErrorArray;
		//				err/=list_length(evalConf);
		//				workingArray[i2] = err;
					workingArray[i2] = meanOfDoubleArray(((ModelInfo *)model->errorsForDropingDecision[i2])->otherErrors[i2], model->sizeOfErrorArray)/(modelGraphIdx->maxid+1);
				}

			}
			tempGreedyEval = EvaluateConfiguration(workingArray, lcpError, modelGraphIdx->models-2, modelGraphIdx->maxid+1);

		//		if(tempGreedyEval < currentGreedyEval)
		//			ereport(NOTICE,(errcode(ERRCODE_SUCCESSFUL_COMPLETION),errmsg("Model %s ist nicht mehr gut mit res=%lf.", model->modelName, tempGreedyEval)));

			if((tempGreedyEval < currentGreedyEval) && (model->sizeOfErrorArray == model->upperBound))
			{
				++model->deletionTicketCnt;

				//if the current model has collected maaaany tickets it is dropped
				if(model->deletionTicketCnt == model->upperBound)
				{
					//check where the model has an entry for exchange
					disAggList = GetAllDisaggregaties((ModelGraphIndexNode *)model->mix);

					foreach(lc1, disAggList)
					{
						if(!((ModelGraphIndexNode *)lfirst(lc1))->disAggModels)
							continue;
						dam = linitial(((ModelGraphIndexNode *)lfirst(lc1))->disAggModels);

						//this disaggregation uses the model we are going to drop
						if(dam->model == model)
						{
							//a secondbest was set, so use this from now on
							if(model->errorsForDropingDecision[((ModelGraphIndexNode *)lfirst(lc1))->id])
							{
								dam->model = model->errorsForDropingDecision[((ModelGraphIndexNode *)lfirst(lc1))->id];
								if(dam->sizeOfErrorArray==dam->model->sizeOfErrorArray)
								{
									dam->errorArray[dam->sizeOfErrorArray-1] = dam->model->otherErrors[((ModelGraphIndexNode *)lfirst(lc1))->id][model->sizeOfErrorArray-1];
								}
								else
								{
									dam->errorArray[dam->sizeOfErrorArray-1] = dam->model->otherErrors[((ModelGraphIndexNode *)lfirst(lc1))->id][model->sizeOfErrorArray];
								}
							...
							/*
							 * when there was no secondbest model for this disAgg use the LCP
							 * this happens when the model in the mgin is the best for forecasting and the DisAgg of the model we are going to drop is the best alternative
							 * we can do this safely here because the model IS used for forecasting and after the next INSERT the best DisAgg would be set again
							 */
							{
								dam->model = ((ModelInfo *)linitial(modelGraphIdx->leafArray[modelGraphIdx->maxid]->models));
								if(dam->sizeOfErrorArray==dam->model->sizeOfErrorArray){
									dam->errorArray[dam->sizeOfErrorArray-1] = dam->model->otherErrors[((ModelGraphIndexNode *)lfirst(lc1))->id][model->sizeOfErrorArray-1];
								}else{
									dam->errorArray[dam->sizeOfErrorArray-1] = dam->model->otherErrors[((ModelGraphIndexNode *)lfirst(lc1))->id][model->sizeOfErrorArray];
						...
						/*
						 * if the DisAgg don't uses the model we are going to drop it might be the secondbest, so we should change this
						 */
						{
							if(dam->model->errorsForDropingDecision[((ModelGraphIndexNode *)lfirst(lc1))->id] == model)
							{
								dam->model->errorsForDropingDecision[((ModelGraphIndexNode *)lfirst(lc1))->id] = ((ModelInfo *)linitial(modelGraphIdx->leafArray[modelGraphIdx->maxid]->models));
							}
						}
					}

					for(i2=0; i2<modelGraphIdx->models;++i2)
					{
						if(modelGraphIdx->modellist[i2] == model)
						{
							break;
						}
					}
					memmove(&(modelGraphIdx->modellist[i2]), &(modelGraphIdx->modellist[i2+1]), (modelGraphIdx->models-i2-1)*sizeof(ModelInfo *));
					--modelGraphIdx->models;

					modelsToReestimate = list_delete_ptr(modelsToReestimate, model);

					//this won't work for several models in one mgin
					elog(INFO, "Model %s dropped.", model->modelName);
					if(list_member_ptr(modelsToReestimate, model))
					{
						list_delete_ptr_wf(modelsToReestimate, model);
					}
					mgin = (ModelGraphIndexNode *)model->mix;
...
//TODO: beim schreiben in memErrorArray und workingArray muss noch mit der aufgezeichneten relativen Anfragehäufigkeit multipliziert werden

	++modelGraphIdx->insertCnt;
...
void triggerMaintenance(int32 number, int32 strategy) //strategy not used yet
{
	List *todelete=NIL;
	List *disAggList=NIL;
	...
	foreach(lc1, modelsToReestimate)
	{
		...
		todelete=lappend(todelete,model);
		ReestimateModelGraphModel(model);
		if(model->maintenance->type!=T_ArOnDemandMaintenance)
		(model->maintenance)->insertCount=0;
//		if(model->maintenance->type==T_SimpleReestMaintenance)
//			((SimpleReestMaintenance*)model->maintenance)->reestInsCnt=model->upperBound;
		disAggList = GetAllDisaggregaties((ModelGraphIndexNode *)model->mix);
		//TODO: momentan nicht in Gebrauch
...
//							dam->sizeOfErrorArray = 0;
	...
	foreach(lc1,todelete)
	{
		modelsToReestimate=list_delete_ptr_wf(modelsToReestimate,lfirst(lc1));
	}
...
//		elog(WARNING, "Reestimated%i @ %i, %lf, %lf", ((ModelGraphIndexNode *)model->mix)->id, model->timestamp, model->errorSMAPE, model->errorReference);
...
{
	if(!modelGraphFillingContext) return;

	MemoryContextSwitchTo(oldContext);
	(*modelGraphFillingContext->methods->delete)(modelGraphFillingContext);
	(*metalGraphContext->methods->delete)(metalGraphContext);
//	EmptyModelGraph();
//	MemoryContextDelete(metalGraphContext);
	modelsToReestimate = NIL;
	modelGraphFillingContext = NULL;
	metalGraphContext = NULL;
	modelGraphRoot=NULL;
	modelGraphIdx=NULL;
	leafCount=0;
}
...
void _insertModelIntoModelgraph(ModelGraphIndexNode *mgin,Datum* values,Relation p)
{
	ModelInfo *newNode=initModelInfo((const char*)DatumGetCString(DirectFunctionCall1(nameout, values[7])),(const char*)DatumGetCString(DirectFunctionCall1(nameout, values[3])),0,NIL);
	...
	initForecastModel(newNode,getModelGraphFillingContext());
...
void _restoreModels(Oid fillid,ModelGraphIndexNode** mginIndex)
{
	...
	while((tuple = heap_getnext(result, ForwardScanDirection)) != NULL)
	{
		...
		heap_deform_tuple(tuple, RelationGetDescr(r), values, isNull);
		_insertModelIntoModelgraph(mginIndex[DatumGetInt16(values[1])],values,p);
		pfree(values);
		pfree(isNull);
	}
...
if(mgin->type != T_AggNode){
	...
	case 21:
	case 23:
		appendStringInfo(res , "%s = %li", mgin->target->resname, GetDatumAsInt(typeOid, mgin->value));
		break;
	case 1042:
...
int FindModelGraphModels(ModelInfo* modelinfo,List **resultList,List **possiblePaths, List **createModels,List *rtable,Node **whereExpr,int explicitCreateModel) {
	...
	root->mgin=getModelGraphRoot(true);
...
//	elog(INFO,"Found %i unique ModelNodes",uniqueResultMgins->length);
	...
	foreach(lc666,uniqueResultMgins) { //fetch models
		ModelGraphIndexNode *mgin=lfirst(lc666);
		...
		{
			if(searchHit->errorSMAPE<searchHit2->errorSMAPE)
			{
				searchHit2 = NULL;
			}
			else
		...
		if(explicitCreateModel!=-2) {

			if(searchHit2) {
				elog(INFO, "USE DISAGG");
				int found=0;
...
//		}
//
//		if(mgin->disAggModels!=NIL && explicitCreateModel!=-2) {
//			DisAggModel *searchHit2= _getDisAgg(mgin,modelinfo);
//			if(searchHit2) {
//				int found=0;
//				//Maybe the model already exists, therefore the disaggkey should be increase
...
//		}
		...
		//Find aggs
		if((mgin->aggIndicator && explicitCreateModel!=-2)) {
				...
				if(!foundagg) {
					...
					if(resultWhere==NULL)
					{
						...
						arforeach(alc1,((ModelGraphIndexNode*)arlfirst(arlist_head(iter->parents)))->children)
						{
							...
							FindModelGraphModels(modelinfo,&result1,&possiblePath, &createModels2,rtable,&(buildWhere),explicitCreateModel);
...
PrintNode2(ModelGraphIndexNode *mgin,StringInfo str)
{
	...
	arforeach(alc, mgin->children) {
		...
		if(nodeTag(mgin)==T_AggNode && tempMgin->parents->length>1) {
			...
			case 23: {
				if(tempMgin->children)
					appendStringInfo(str, "node%p [label=\"{%s|%li}\"];\n", tempMgin, tempMgin->target->resname, GetDatumAsInt(typeOid,tempMgin->value));
				else {
					appendStringInfo(str, "node%p [mginid={%i},label=\"{%s|%li}\", modelCount={%i}", tempMgin, tempMgin->id,tempMgin->target->resname, GetDatumAsInt(typeOid,tempMgin->value),list_length(tempMgin->models)+list_length(tempMgin->disAggModels));
...
bool isModelGraphExistent(void)
{
	bool result;
	ModelGraphIndexNode *mgin;
	if(!strcmp(CurrentMemoryContext->name, "ModelGraphFillingContext")) //TODO: IST DAS CLEVER? -> Jetzt vlt.
		return true;
	mgin=getModelGraphRoot(false);
	if(mgin->children)
		result=true;
