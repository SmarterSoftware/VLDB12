...
double arima_nonSeasonal_ML(unsigned n, const double *x, double *grad, void *my_func_data) {
	...
	for(i=(p); i<model->obsCount; i++) {
		for(variable=0; variable<n; variable++) {
			double tmpVarUp=valup[variable]/(n-p-q);
			double tmpVarDown=valdown[variable]/(n-p-q);
			valup[variable]=0;
			valdown[variable]=0;
...
double arima_AddSeasonal_CSS(unsigned n, const double *x, double *grad, void *my_func_data) {
	...
	for(i=Max(1*model->super.p,model->super.sp*model->sflag); i<model->obsCount; i++) {
		...
		if(f<0) elog(ERROR,"MWAHAHAA BUMM");
		if((tempOverFlowCheck>0 && oldf>f) ||(tempOverFlowCheck<0 && oldf<f)  ){
			f=DBL_MAX; //Overflow happend
			elog (WARNING,"Overflow detected. Codekey:ar_ss_smape2");
		}
...
double arima_AddSeasonal_ML(unsigned n, const double *x, double *grad, void *my_func_data) {
	...
	double varianz,varianzUp,varianzDown;
	...
	for(i=Max(1*model->super.p,model->super.sp*model->sflag); i<model->obsCount; i++) {
		for(variable=0; variable<n; variable++) {
			double tmpVarUp=valup[variable]/(n-model->super.p-model->super.q-model->super.sp-model->super.sq);
			double tmpVarDown=valdown[variable]/(n-model->super.p-model->super.q-model->super.sp-model->super.sq);
			valup[variable]=0;
			valdown[variable]=0;
...
initarModel(PG_FUNCTION_ARGS) {
	...
	m->calculateTrustedInterval = &calculateArTrustedInterval;
	model->model=(Model *)m;
...
void
identifyArimaModel(ARModel *model) {
	int			nelemsp;
	ArrayType	*array;
	ArrayType	*array2;
	List		*funcname;
	...
	Datum 		result;
	Datum 	    *parameters;
	Datum * helpTransportArray=palloc0(sizeof(Datum)*model->obsCount);

	elog(LOG, "start auto arima");

	for(nelemsp=0;nelemsp<model->obsCount;nelemsp++)
		helpTransportArray[nelemsp]=Float8GetDatum(model->data[1][nelemsp]);
	// construct array from data
	array = construct_array(helpTransportArray, model->obsCount, FLOAT8OID, sizeof(float8), FLOAT8PASSBYVAL, 'i');

	// get the oid of the autoArima function
	funcname = list_make1(makeString("autoarima"));
	funcargtypes[0] = FLOAT8ARRAYOID;
	funcargtypes[1] = INT4OID;
	funcoid = LookupFuncName(funcname, 2, funcargtypes, false);

	// call autoArima function
	result = OidFunctionCall2(funcoid, PointerGetDatum(array), Int32GetDatum(model->super.pd));
	array2 = DatumGetArrayTypeP(result);

	// set parameters
	deconstruct_array(array2, FLOAT8OID, sizeof(float8), FLOAT8PASSBYVAL, 'i', &(parameters), NULL, &nelemsp);

	model->super.p = (int) DatumGetFloat8(parameters[0]);
	model->super.q = (int) DatumGetFloat8(parameters[1]);
	model->super.sp = (int) DatumGetFloat8(parameters[2]);
	model->super.sq = (int) DatumGetFloat8(parameters[3]);
	model->super.d = (int) DatumGetFloat8(parameters[5]);
	model->super.sd = (int) DatumGetFloat8(parameters[6]);


pfree(helpTransportArray);
	elog(LOG, "Parameters: %i %i %i %i %i %i", model->super.p, model->super.q, model->super.sp, model->super.sq, model->super.d, model->super.sd);
}
...
finalizearModel(PG_FUNCTION_ARGS) {
	...
	// check if model parameters are determined using autoArima
	if (model->super.automatic == 1)
		identifyArimaModel(model);
	// set automatic to 0 so it is not done again for re-estimation
	model->super.automatic = 0;

	old = MemoryContextSwitchTo(model->dataCtx);
	double zs[model->super.sp+model->super.sq+model->super.includeConstant+model->super.p+model->super.q];

	maxArCount=Max(model->super.p,(model->super.sp*model->super.pd)); //how many function values should be stored
	if(model->super.phis)  //we are reestimating, therefor some structures already exist
	{
			pfree(model->super.y);
			if(model->super.uhat)
				pfree(model->super.uhat);
	}
	model->super.y = palloc0(maxArCount*sizeof(double));

	if(model->super.q || model->super.sq)
		model->super.uhat=palloc0(((Max(model->super.q,(model->super.sq*model->super.pd))))*sizeof(double)); 
	if(!model->super.phis)
	...
	if(model->super.d)
	{
		model->super.diffhelp_ns=palloc0(sizeof(double)*model->super.d);
		model->super.diffhelp_temporal_ns=palloc0(sizeof(double)*(model->super.d));
		...
		for(i=0; i<model->super.d; i++)
		{
			...
			{
				if(j==model->obsCount-1)
					model->super.diffhelp_ns[model->super.d-1-i]=model->data[1][model->obsCount-1]; 
	...
	if(model->super.sd)
	{
		model->super.diffhelp_s=palloc0(sizeof(double*)*model->super.sd);
		model->super.diffhelp_temporal_s=palloc0(sizeof(double*)*(model->super.sd));

		for(i=0; i<model->super.sd; i++)
		{
			model->super.diffhelp_s[i]=palloc0(sizeof(double)*(model->super.pd+1));
			model->super.diffhelp_temporal_s[i]=palloc0(sizeof(double)*(model->super.pd+1));
			for(j=model->obsCount-1; j>model->super.pd; j--)
			{
				if(j>=model->obsCount-1-model->super.pd)
					model->super.diffhelp_s[i][j-(model->obsCount-1-model->super.pd)]=model->data[1][j]; 
	...
	//calculation non-seasonal Arima if needed
		...
		double ubns[model->super.p+model->super.q+model->super.sq+model->super.sp+model->super.includeConstant];
		double lbns[model->super.p+model->super.q+model->super.sq+model->super.sp+model->super.includeConstant];
	...
	elog(WARNING,"Optimization time: %f",diff);
	...
	for(i=0; i<model->super.p+model->super.sp; i++) {
		elog(INFO,"Phi %i: %lf",i,zs[i]);
	}
	for(i=model->super.p+model->super.sp; i<model->super.p+model->super.sp+model->super.q+model->super.sq; i++) {
		elog(INFO,"Theta %i: %lf",i,zs[i]);
	}

	model->super.error=minf;
	...
	double f;
	 initArimaForecast(&(model->super)); //init model
	 f = _get_next_Arimaforecast(&(model->super),model->super.phis,model->super.thetas,model->super.uhat);
...
void incrementalUpdatearModel1(PG_FUNCTION_ARGS) {
	elog(ERROR,"operation no longer supported supported.Codekey:ar2iu");
}
...
void incrementalUpdatearModel(PG_FUNCTION_ARGS) {
	...
	ArimaModel *forecast_model = &(((ARModel *)PG_GETARG_POINTER(0))->super);
	double ynew = PG_GETARG_FLOAT8(1);
...
if(forecast_model->ycount <= Max(forecast_model->p,forecast_model->sp*pd)+forecast_model->actualForecast)
	...
	if(forecast_model->d)
	{
		...
		{
			forecast_model->y[start+forecast_model->p+(forecast_model->sp*forecast_model->pd)-1]-=forecast_model->diffhelp_temporal_ns[i];
			forecast_model->diffhelp_temporal_ns[forecast_model->d-i-1]=ynew;
		}
	...
	if(forecast_model->sd)
	{
		...
		{
			forecast_model->y[start+forecast_model->p+(forecast_model->sp*forecast_model->pd)-1]-=forecast_model->diffhelp_temporal_s[i][0];
			forecast_model->diffhelp_temporal_s[i][forecast_model->pd]=ynew;
	...
	memmove(&(forecast_model->y[0]),&(forecast_model->y[1]),sizeof(double)*(forecast_model->ycount-1));
	if(forecast_model->uhat)
	{
		if(model->maintenance->type==T_ArOnDemandMaintenance)
		{
			ArOnDemandMaintenance* aodm=(ArOnDemandMaintenance*)model->maintenance;
			int i;
			for(i=0;i<(forecast_model->p+forecast_model->sp+forecast_model->q+forecast_model->sq+forecast_model->includeConstant);i++)
			{
				updateArUhat(aodm->hyperCubeUhatsDown[i],forecast,forecast_model->y[start+jump-1],Max(forecast_model->q,(forecast_model->sq*forecast_model->pd)));
				updateArUhat(aodm->hyperCubeUhatsUp[i],forecast,forecast_model->y[start+jump-1],Max(forecast_model->q,(forecast_model->sq*forecast_model->pd)));
			}
				
		}
		updateArUhat(forecast_model->uhat,forecast,forecast_model->y[start+jump-1],Max(forecast_model->q,(forecast_model->sq*forecast_model->pd)));
	}
...
void updateArUhat(double* uhat, double forecast,double realValue, int uhatLength)
{
//	elog(WARNING,"....");
	if(!uhat) return;
//		elog(WARNING,"old uhat: %lf, new uhat: %lf",uhat[uhatLength-1],forecast-realValue);
	memmove(&(uhat[1]),&(uhat[0]),(uhatLength-1)*sizeof(double));
	uhat[0]=forecast-realValue;
	
}


void incrementalUpdatearModel2(PG_FUNCTION_ARGS){
	elog(ERROR,"operation not supported.Codekey:ar2iu2");
}
...
double calculateArTrustedInterval(Model *model, int k, double percentage)
{
	ereport(ERROR,(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),errmsg("Prediction intervals for ARIMA-MODELS are not implemented yet!")));
