...
inithwModel(PG_FUNCTION_ARGS) {
	...
	m->calculateTrustedInterval = &calculateHwTrustedInterval;
	model->model=(Model *)m;
...
void incrementalUpdatehwModel1(PG_FUNCTION_ARGS) {
	...
	int64 timestamp = PG_GETARG_INT32(2);
	...
	((HWModel *)PG_GETARG_POINTER(0))->expectedValueX+=result;
	((HWModel *)PG_GETARG_POINTER(0))->expectedValueXSquared+=pow(result,2);
	((HWModel *)PG_GETARG_POINTER(0))->obsCount++;
...
finalizehwModel(PG_FUNCTION_ARGS) {
	...
	/***********************************************************************/
	/*Palloc some structures for optimization, 2 trend and level values would be
	...
	{		
		...
		//if there are not enough observations  we can't calculate the starting trend
		if(model->obsCount<model->super.start_time+1)
		{
			elog(ERROR,"not enough observations. CODEKEY:finalizehw1, %s, %i", __FILE__, __LINE__);
		}
...
//		model->super.b=trend42seasons;
		model->super.b=(model->data[2]-model->data[0])/model->super.start_time;
	...
	for(j=0;j<model->super.start_time;j++)
	{
		temp=temp+model->data[j];
	}
	avgOnePeriod=temp/model->super.start_time;
	...
	{
		//if there are not enough observations  we can't calculate the starting trend
		if(model->obsCount<model->super.start_time*2)
		{
			elog(ERROR,"not enough observations. CODEKEY:finalizehw1, %s, %i", __FILE__, __LINE__);
		}
		...
		{
			//additive
			for(i=0;i<model->super.start_time;i++)//anzahl periodenlänge viele
			{
		...
		{
			//multiplicative
			for(i=0;i<model->super.start_time;i++)
			{
 ...
 * k - the the horizon
 */
double calculateHwTrustedInterval(Model *model, int k, double percentage)
{
	HWModel *hwmodel = (HWModel *)model;
	int p = hwmodel->super.period;
	double alpha = hwmodel->super.alpha;
	double beta = hwmodel->super.beta;
	double gamma = hwmodel->super.gamma;
	int s = k/p;

	double t1 = inverse_normal_cdf((1+percentage)/2);
	double t2 = (1 + (k-1)*(alpha*alpha)*(1+k*beta+1/6*k*(2*k-1)*(beta*beta)) + s*((gamma*gamma)*((1-alpha)*(1-alpha))+alpha*gamma*(1-alpha)*(2+p*gamma*(s+1))));
	double t3 = (1.0/(hwmodel->obsCount-1))*(hwmodel->expectedValueXSquared-((1.0/hwmodel->obsCount)*pow(hwmodel->expectedValueX,2)));
