...
initMediumForecastModel(PG_FUNCTION_ARGS)
{
	...
	medModel->calculateTrustedInterval = &calculateMedTrustedInterval;
...
double calculateMedTrustedInterval(Model *model, int k, double percentage)
{
	ereport(ERROR,(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),errmsg("Prediction intervals for MEDIUM-MODELS are not implemented yet!")));
