...
{	
	...
	model->calculateTrustedInterval = &calculateLinTrustedInterval;
...
incrementalUpdateLinRegModel(PG_FUNCTION_ARGS)
{
	LinRegModel *model = (LinRegModel*) PG_GETARG_POINTER(0);
	double value = PG_GETARG_FLOAT8(1);
	ModelInfo *modelInfo=(ModelInfo*) PG_GETARG_POINTER(3);
	
	processForecastModel(modelInfo, value);
...
loadLinRegModelParameters(PG_FUNCTION_ARGS)
{
	...
	elog(ERROR,"STORAGE TABLE is not supported anymore AND this has to be massively fixed before it will word without WARNINGS!!!");
...
double calculateLinTrustedInterval(Model *model, int k, double percentage)
{
	ereport(ERROR,(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),errmsg("Prediction intervals for LINREG-MODELS are not implemented yet!")));
