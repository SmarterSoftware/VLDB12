...
initRModel(PG_FUNCTION_ARGS)
{
	...
	// init parameters
	if(modelInfo->parameterList)
		parseRParameters(modelInfo->parameterList,model);
	else
		model->period = 1;
...
finalizeRModel(PG_FUNCTION_ARGS)
{
	...
	funcargtypes[2] = INT4OID;
	funcoid = LookupFuncName(funcname, 3, funcargtypes, false);

	// Call etsforecast function
	result = OidFunctionCall3(funcoid, PointerGetDatum(array), Int32GetDatum(model->numForecastValues), Int32GetDatum(model->period));
...
void
parseRParameters(List *parameterList, RModel *specificModel) {
	ListCell				*cell;

	foreach(cell,parameterList) {
		AlgorithmParameter		*param = lfirst(cell);

		if(strcmp(param->key,"season") == 0) {
