...
transformDeleteStmt(ParseState *pstate, DeleteStmt *stmt)
{
	...
	transformFromClause(pstate, stmt->usingClause, &t);
...
transformSelectStmt(ParseState *pstate, SelectStmt *stmt)
{
	...
	transformFromClause(pstate, stmt->fromClause,&all_array);
	...
	if (stmt->forecastExpr)
	{
		if (IsA(stmt->forecastExpr, ForecastExpr)) {
			...
			stmt->forecastExpr = (Node *)findTimeAndMeasureColumns(pstate, (ForecastExpr *)stmt->forecastExpr, qry);
			//TODO: check if there are "ORDER BY time" clauses, and if not add them
			if ((!stmt->sortClause) && (all_array == 0 ))
			{
				SortBy *sortByTime = makeNode(SortBy);
				sortByTime->node = copyObject(((ForecastExpr*)stmt->forecastExpr)->time);
				sortByTime->sortby_dir = SORTBY_DEFAULT;
				sortByTime->sortby_nulls = SORTBY_NULLS_DEFAULT;
				sortByTime->useOp = NIL;
				sortByTime->location = -1;

				stmt->sortClause = lappend(stmt->sortClause, sortByTime);
			}
		}
	}
	/*
...
transformUpdateStmt(ParseState *pstate, UpdateStmt *stmt)
{
	 ...
	 */
	transformFromClause(pstate, stmt->fromClause, &t);
