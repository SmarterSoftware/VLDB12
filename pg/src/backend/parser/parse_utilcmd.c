...
transformCreateDisAggSchemeStmt(CreateDisAggSchemeStmt *stmt, const char *queryString, char *completionTag){
	...
	//for ColumnRefs with prepended table (Table.Attribute)
	pstate->p_relnamespace = rTable;
	//for ColumnRefs without prepended table (Attribute)
...
transformFillModelGraphStmt(FillModelGraphStmt *stmt, const char *queryString, char *completionTag){
	...
	stmt->sourcetext = palloc0((strlen(queryString)+1)*sizeof(char));
	stmt->algorithm->algorithmname = getModelTypeAsString(getStringAsModelType(stmt->algorithm->algorithmname));
	strcpy((char *)stmt->sourcetext, queryString);
