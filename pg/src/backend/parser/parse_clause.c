 ...
 */
void
transformFromClause(ParseState *pstate, List *frmList, int *all_array)
{
	ListCell   *fl;
	*all_array=1;
	/*
...
transformFromClauseItem(ParseState *pstate, Node *n,
						RangeTblEntry **top_rte, int *top_rti,
						List **relnamespace,
						Relids *containedRels, int* is_array)
{
	if (IsA(n, RangeVar))
	{
		...
		*is_array=0;
		if (rte->relkind==RELKIND_ARRAY) *is_array=1;
	
		//elog(WARNING,"relkind %d %d",rte->relkind, *is_array);
		/* assume new rte is at end */
		rtindex = list_length(pstate->p_rtable);
		Assert(rte == rt_fetch(rtindex, pstate->p_rtable));
		*top_rte = rte;
...
ForecastExpr *findTimeAndMeasureColumns(ParseState *pstate, ForecastExpr *expr, Query *query)
{
		ListCell   		*tl;
		...
		Oid 			type;
		ColumnRef *colrT = makeNode(ColumnRef);
		ColumnRef *colrM = makeNode(ColumnRef);
		...
		else
		{
			foreach(tl, query->targetList)
			{
				...
				if (exprType((Node*) tle->expr) == DATEOID
					||exprType((Node*) tle->expr) == TIMESTAMPOID
					||exprType((Node*) tle->expr) == INT2OID
					||exprType((Node*) tle->expr) == INT4OID
					||exprType((Node*) tle->expr) == INT8OID)
				{
					timeCol = tle;
		...
		colrT->fields = lappend(colrT->fields,makeString(get_tle_by_resno(query->targetList,linitial_int(timeCols))->resname));
		colrT->location = -1;

		colrM->fields = lappend(colrM->fields,makeString(get_tle_by_resno(query->targetList,linitial_int(measureCols))->resname));
		colrM->location = -1;
		
		expr->time = (Node *)colrT;
		expr->measure = (Node *)colrM;
		...
		foreach(tl, query->targetList)
		{
			...
			if ((!list_member_int(expr->timeCols, tle->resno)) && (!list_member_int(expr->measureCols, tle->resno)))
			{
				/*if (type == BYTEAOID)
				{
				...
				{
					ereport(ERROR, (errmsg("error while parsing forecast clause, only string category columns supported")));
				}*/
				ereport(ERROR, (errmsg("category columns not supported!")));
			}
		}
...
{
	TargetEntry 	*tle 	= get_tle_by_resno(query->targetList,linitial_int(expr->measureCols));
	TargetEntry *timeColumn = (TargetEntry*) list_nth(query->targetList, linitial_int(expr->timeCols) - 1);
	...
	if (expr->length == 0) {
		ereport(ERROR, (errcode(ERRCODE_INVALID_PARAMETER_VALUE), errmsg("forecast length must be greater than 0")));
	} else if(expr->length == -1){
		if(exprType(timeColumn->expr)==INT2OID || exprType(timeColumn->expr)==INT4OID || exprType(timeColumn->expr)==INT8OID)
		{
			ereport(ERROR, (errcode(ERRCODE_INVALID_PARAMETER_VALUE), errmsg("For time-columns of type INT it is not allowed to work with intervals!")));
		}
		if(exprType(timeColumn->expr)==DATEOID && expr->granularity < day && expr->targetDateString == 0)
		{
			ereport(ERROR, (errcode(ERRCODE_INVALID_PARAMETER_VALUE), errmsg("For time-columns of type DATE the INTERVAL-granularity must be day or greater!")));
		}
		if((expr->targetDateString == 0 && expr->altLength == 0)) //no targetDate given
		{
			ereport(ERROR, (errcode(ERRCODE_INVALID_PARAMETER_VALUE), errmsg("Either a targetDate or an interval must be given!")));
		}
	}

	// CHECK PREDICTION INTERVALS
	if(expr->confidenceIntervalPercentage) {
		if (atof(expr->confidenceIntervalPercentage->val.str) >= 1.0)
			ereport(ERROR, (errcode(ERRCODE_INVALID_PARAMETER_VALUE), errmsg("ConfidenceIntervalPercentage must be less than 1")));
	}
