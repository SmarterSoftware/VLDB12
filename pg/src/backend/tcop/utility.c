			   ...
			   char *completionTag)
{
	...
	switch (nodeTag(parsetree))
	{
		...
		case T_CreateStmt:
			{
				...
				foreach(l, stmts)
				{
					...
					if (IsA(stmt, CreateStmt))
					{
						...
						//elog(WARNING, "create stmt is array %d", cs->isarray);
						if (cs->isarray)
							relOid = DefineRelation((CreateStmt *)stmt, RELKIND_ARRAY);
		...
		case T_FillModelGraphStmt:
			parsetree = (Node *)transformFillModelGraphStmt((FillModelGraphStmt *)parsetree, queryString, completionTag);

			FillModelGraph((FillModelGraphStmt *)parsetree);
			break;
		case T_ReestimateModelGraphModelsStmt:
			triggerMaintenance(((ReestimateModelGraphModelsStmt *)parsetree)->number,-1);
			break;
...
CreateCommandTag(Node *parsetree)
{
	...
	switch (nodeTag(parsetree))
	{
		...
		case T_CreateStmt:
			tag = "CREATE TABLE OR TIMESERIES"; //may be more specific
			break;
