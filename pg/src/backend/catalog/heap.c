...
static void AddNewRelationTuple(Relation pg_class_desc,
					...
					Datum reloptions);
static void
AddNewArrayTuple(			Oid new_rel_oid,
					int size,
					int number,
					int lastvalue
					);
...
{
	//FormData_pg_array new_rel_reltup;
Relation pg_array_desc= heap_open(ArrayRelationId, RowExclusiveLock);
	//elog(WARNING,"insert array opened");
	Datum		values[Natts_pg_array];
	bool		nulls[Natts_pg_array];
	HeapTuple	tup;
	...
	values[Anum_pg_array_oid - 1] = 	ObjectIdGetDatum(oid);
	values[Anum_pg_array_size - 1] = Int32GetDatum(size);
	values[Anum_pg_array_number - 1] = Int32GetDatum(number);
	values[Anum_pg_array_lastvalue - 1] = Int32GetDatum(lastvalue);


	tup = heap_form_tuple(RelationGetDescr(pg_array_desc), values, nulls);

	/*
	 * The new tuple must have the oid already chosen for the rel.	Sure would
	 * be embarrassing to do this sort of thing in polite company.
	 */
	Oid noid=GetNewOid(pg_array_desc);
	elog(WARNING,"Oid= %d noid=%d",oid,noid);
	HeapTupleSetOid(tup, noid);

	/* finally insert the new tuple, update the indexes, and clean up */
	simple_heap_insert(pg_array_desc, tup);

	//CatalogUpdateIndexes(pg_array_desc, tup);

	heap_freetuple(tup);


	// Close relation
	heap_close(pg_array_desc, RowExclusiveLock);
...
/*int tuples_per_page(Relation r){
int i;
 TupleDesc   desc=r->rd_att;
bool nulls[3];
Datum values[3];
	...
	s=s+att->attlen;
	nulls[i] = false;
        values[i] = Int8GetDatum(1);;
 }
elog(WARNING,"size %d", s);
elog(WARNING,"natts %d", desc->natts);
   int  len = offsetof(MinimalTupleData, t_bits);
   ...
   int  data_len = heap_compute_data_size(desc, values, nulls);

    len += data_len;

HeapTuple h=heap_form_tuple(desc,values,nulls);
						 ...
						 bool allow_system_table_mods)
{
	...
	if (relkind == RELKIND_ARRAY) {
         //tuples_per_page(new_rel_desc);
         AddNewArrayTuple(relid, 0,0,0);
         UpdateLastValue(relid,0,0);
	}
