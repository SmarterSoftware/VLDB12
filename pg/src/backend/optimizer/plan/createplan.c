...
make_forecast(Plan *lefttree, Node *forecastExpr, Query **qry)
{
	...
	List 				*createModels = NIL, *possiblePath = NIL;
	ModelInfo			*modelInfo;
	bool 				found = false;
		
	// TODO: COST MODEL
	copy_plan_costsize(plan, lefttree);
	plan->plan_rows = forecast->length; // FIX: works only if we just forecast one time series, otherwise: horizon * number_of_distinct_attributes
	...
	node->targetDateString = forecast->targetDateString; //foreward targetDate...
	node->altLength = forecast->altLength; //...and the altLength...
	node->targetGranularity = forecast->granularity; //and the granularity
	...
	initMaintenanceMethod(modelInfo,forecast->maintenancetype,forecast->maintenanceParameters);
	...
	if((forecast->storeModel == 4) && (modelInfo->forecastMethod != R)) // MODELGRAPH
	{
		...
		FindModelGraphModels(modelInfo,&node->candidatemodelInfos,&possiblePath, &createModels,(*qry)->rtable,&(forecast->whereExpr), forecast->length);
	...
	if ((forecast->storeModel == 0) && (modelInfo->forecastMethod != R)) // MODELINDEX
	{
	...
	else if ((forecast->storeModel == 2) && (modelInfo->forecastMethod != R)) // HASHTABLE
	{
	...
	else if ((forecast->storeModel == 3) && (modelInfo->forecastMethod != R)) // SYSTEMTABLE
	{
		//Get list of possible models
	...
	// NO MODEL FOUND: add a CreateModel Plannode
	if (!node->candidatemodelInfos) {

		Plan *createModel;
		modelInfo->buildModel = true;

		// MODELGRAPH: Add the model, we plan to create to the candidates
		if((createModels)!=NIL)
		{
	...
	if(forecast->confidenceIntervalPercentage) {
		((ModelInfo *)linitial(node->candidatemodelInfos))->confidenceIntervalPercentage = atof(forecast->confidenceIntervalPercentage->val.str);
		if (((ModelInfo *)linitial(node->candidatemodelInfos))->confidenceIntervalPercentage > 0.0)
		{
			TargetEntry *tle = makeTargetEntry(((ModelInfo *)linitial(node->candidatemodelInfos))->measure->expr,3,"trusted interval",false);
			plan->targetlist = lappend(plan->targetlist, tle);
		}
	} else {
		((ModelInfo *)linitial(node->candidatemodelInfos))->confidenceIntervalPercentage = 0.0;
	}
