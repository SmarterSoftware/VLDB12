...
set_plain_rel_pathlist(PlannerInfo *root, RelOptInfo *rel, RangeTblEntry *rte)
{
	...
	/* Consider sequential scan */
        Path *seqscan_path = create_seqscan_path(root, rel);
	if (rte->relkind == RELKIND_ARRAY) {
		seqscan_path->startup_cost = 0; 
		seqscan_path->total_cost = 0;
	}
		
	add_path(rel, seqscan_path);
